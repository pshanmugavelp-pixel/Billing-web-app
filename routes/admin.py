...
def update_seller_info():
    ...
    with db_connection() as conn:
        try:
            ...
            conn.rollback()
            current_app.logger.exception('Error updating seller information')
            flash('Could not update seller information. Please try again.', 'error')
            return redirect(url_for('admin.seller_info'))
...
def view_table(table_name):
    ...
    with db_connection() as conn:
        try:
            ...
            current_app.logger.exception('Error viewing table %s', table_name)
            flash('Could not load the table view. Please try again.', 'error')
            return redirect(url_for('admin.index'))
...
def delete_rows(table_name):
    ...
    with db_connection() as conn:
        try:
            ...
            conn.rollback()
            current_app.logger.exception('Error deleting rows from table %s', table_name)
            flash('Could not delete the selected rows. Please try again.', 'error')
...
def reset_table(table_name):
    ...
    with db_connection() as conn:
        try:
            ...
            conn.rollback()
            current_app.logger.exception('Error resetting table %s', table_name)
            flash('Could not reset the table. Please try again.', 'error')
...
def reset_all():
    ...
    with db_connection() as conn:
        try:
            ...
            conn.rollback()
            current_app.logger.exception('Error resetting all tables')
            flash('Could not reset tables. Please try again.', 'error')
